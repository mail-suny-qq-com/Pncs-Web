  ...
  <div class="app-container">
    ...
    <div class="head-container">
      ...
      <el-table ref="table" v-loading="crud.loading" :data="crud.data" size="small" style="width: 100%;" @selection-change="crud.selectionChangeHandler">
        ...
        <el-table-column v-if="columns.visible('ieProp')" prop="ieProp" label="指标类别(1-财务类，2-经营类...)">
          <template slot-scope="scope">
            {{ dict.label.IE_PROP[scope.row.ieProp] }}
          </template>
        </el-table-column>
        <el-table-column v-if="columns.visible('ieType')" prop="ieType" label="指标类型(0-手工指标，1-基础指标，2-衍生指标，3-定性指标)">
          <template slot-scope="scope">
        ...
        <el-table-column v-if="columns.visible('ieMethod')" prop="ieMethod" label="取值方式( 衍生指标时：1-四则运算，2-复杂SQL，3-限制，4-EXCEL公式)">
          <template slot-scope="scope">
            {{ dict.label.IE_METHOD[scope.row.ieMethod] }}
          </template>
        </el-table-column>
        <el-table-column v-if="columns.visible('calcFreq')" prop="calcFreq" label="计算频度(D-天，M-月，Q-季度，Y-年)">
          <template slot-scope="scope">
            {{ dict.label.CALC_FREQ[scope.row.calcFreq] }}
          </template>
        </el-table-column>
        <el-table-column v-if="columns.visible('status')" prop="status" label="指标状态（0-已创建，1-已提交，2-已审批）">
          <template slot-scope="scope">
        ...
        <el-table-column v-if="columns.visible('ieLevel')" prop="ieLevel" label="指标层级(根据依赖关系确定层级，基础指标为0)" />
        ...
        <el-table-column v-if="columns.visible('manageDept')" prop="manageDept" label="管理部门(解释部门)" />
